...
public class ProductsFilterController {
	...
	@GetMapping("/cgProductLists/accom-type/{accom-type}")
	public String cgProductListByAccomtype(@AuthenticationPrincipal PrincipalDetail userDetail,
										   @PathVariable("accom-type") String accom_type, 
										   Accom accom, 
										   Model model) {
		
		log.info("리펙토링 cgProductByAccomtype 시작");
		log.info("리펙토링 cgProductByAccomtype accom_type-> {} ", accom_type);
		...
		log.info("로그인하고 안하고 확인하기 user_id ->  {}", user_id);
		log.info("로그인하고 안하고 확인하기 auth ->  {}", auth);
	...
	public String cgProductListsByAddr(@AuthenticationPrincipal PrincipalDetail userDetail,
		...
		log.info("컨트롤러 cgProductListsByAddr 사이즈-> {} ", cgProductListsByAddr.size());
